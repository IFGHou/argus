...
/* 
 * $Id: //depot/argus/argus/argus/ArgusModeler.c#111 $
 * $DateTime: 2012/03/15 01:13:08 $
 * $Change: 2335 $
 */
...
struct teredo {
   ...
   union {
      ...
      struct teredoOriginHdr  torigin;
   } t_un;
};
      ...
      if (!((sport == 53) || (dport == 53))) {
         ...
         if (STRUCTCAPTURED(model, *tptr)) {
            ...
            switch (type) {
               ...
               case 0x0001:  offset = (4 + (tptr->t_un.tauth.idlen + tptr->t_un.tauth.aulen) + 8 + 1); break;
               default: isipv6 = -1;
            }
   ...
   if (!(length) && !(tvp) && !(lcp))
      ...
      if ((tflow = ArgusCreateLcpFlow(model, lcp)) != NULL) {
         ...
         if ((flow = ArgusFindFlow (model, model->hstruct)) == NULL) {
            flow = ArgusNewFlow(model, model->ArgusThisFlow, model->hstruct, model->ArgusStatusQueue);
...
ArgusCreateIPv6Flow (struct ArgusModelerStruct *model, struct ip6_hdr *ip)
{
   ...
   if ((ip != NULL) && STRUCTCAPTURED(model, *ip)) {
      ...
      len = sizeof(ip->ip6_src)/sizeof(int);
      if (model->ArgusFlowType == ARGUS_BIDIRECTIONAL) {
         while ((i < len) && (*dp == *sp)) {    /* copy while they are equal */
...
setArgusFarReportInterval (struct ArgusModelerStruct *model, char *value)
{
   ...
   if (((ptr = strchr(value, '.')) != NULL) || isdigit(i)) {
      ...
      if (ptr != NULL) {
         ...
         model->ArgusFarReportInterval.tv_usec = abs(remainderf(fvalue, 1.0) * 1000000);
