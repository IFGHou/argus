...
/*
 * $Id: //depot/argus/argus/argus/ArgusOutput.c#68 $
 * $DateTime: 2012/04/13 15:41:51 $
 * $Change: 2363 $
 */
...
#ifdef ARGUSDEBUG
                           ArgusDebug (5, "ArgusOutputProcess() %d client(s) not ready fd %d sock %p start %d", output->ArgusClients->count, client->fd, client->sock, client->ArgusClientStart);
#endif
                        }
                        client = (void *) client->qhdr.nxt;
                     }
...
ArgusCheckClientMessage (struct ArgusOutputStruct *output, struct ArgusClientData *client)
{
   ...
   if ((cnt = recv (fd, buf, value, 0)) <= 0) {
      if (cnt < 0) {
#ifdef ARGUSDEBUG
         ArgusDebug (6, "ArgusCheckClientMessage (%p, %p) recv(%d) returned error %s\n", output, client, fd, strerror(errno));
#endif
         return (-1);
         ...
         switch (i) {
            ...
            case RADIUM_FILTER: {
               if (ArgusFilterCompile (&client->ArgusNFFcode, &ptr[7], 1) < 0) {
                  retn = -2;
#ifdef ARGUSDEBUG
                  ArgusDebug (6, "ArgusCheckClientMessage: ArgusFilter syntax error: %s\n", &ptr[7]);
#endif
               } else {
#ifdef ARGUSDEBUG
                  char buf[MAXSTRLEN];
                  ...
                  if ((cnt = send (fd, "OK", 2, 0)) != 2) {
                     retn = -3;
#ifdef ARGUSDEBUG
                     ArgusDebug (6, "ArgusCheckClientMessage: send error %s\n", strerror(errno));
#endif
                  } else {
                     retn = 0;
#ifdef ARGUSDEBUG
                     ArgusDebug (6, "ArgusCheckClientMessage: ArgusFilter %s initialized.\n", &ptr[7]);
#endif
                  }
               }
               break;
...
ArgusGenerateInitialMar (struct ArgusOutputStruct *output)
{
   ...
   if ((ArgusSrc = output->ArgusSrc->srcs[0]) != NULL) {
      retn->argus_mar.localnet = ArgusSrc->ArgusInterface[0].ArgusLocalNet;
      retn->argus_mar.netmask  = ArgusSrc->ArgusInterface[0].ArgusNetMask;
   }
