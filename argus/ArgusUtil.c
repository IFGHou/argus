...
/* 
 * $Id: //depot/argus/argus/argus/ArgusUtil.c#70 $
 * $DateTime: 2012/03/15 00:43:34 $
 * $Change: 2332 $
 */
...
ArgusUpdateTime (struct ArgusModelerStruct *model)
{
   ...
   ival = ((model->ArgusUpdateInterval.tv_sec * 1000000LL) + model->ArgusUpdateInterval.tv_usec);
...
ArgusCreateFlowKey (struct ArgusModelerStruct *model, struct ArgusSystemFlow *flow, struct ArgusHashStruct *hstruct)
{
   ...
   if (len > 0) {
      ...
      if (model->ArgusFlowKey & ARGUS_FLOW_KEY_VLAN) {
         *key++ = model->ArgusThisPacket8021QEncaps;
         hstruct->len++;
      }
   
      if (model->ArgusFlowKey & (ARGUS_FLOW_KEY_LOCAL_MPLS |
                                 ARGUS_FLOW_KEY_COMPLETE_MPLS)) {
      ...
      if (ArgusListEmpty (list) && (asock->rec == NULL)) {
         ...
         if (!(output->ArgusWriteStdOut) && (asock->filename)) {
            if (asock->lastwrite.tv_sec < output->ArgusModel->ArgusGlobalTime.tv_sec) {
               if (((stat (asock->filename, &statbuf)) < 0) || (asock->fd != -1) || (ArgusCloseFile)) {
                  if (asock->fd != -1)
                     close(asock->fd);
...
#ifdef ARGUSDEBUG
                     ArgusDebug (6, "ArgusWriteSocket: created outfile %s\n", asock->filename);
#endif
               }
...
#define ARGUS_MAX_TOSS_RECORD	64
         ...
         if (asock->rec == NULL)
            asock->rec = (struct ArgusRecordStruct *) ArgusPopFrontList(list, ARGUS_NOLOCK);
                        ...
                        if (((stat (asock->filename, &statbuf)) < 0) || (ArgusCloseFile)) {
                           ...
                           if ((asock->fd = open (asock->filename, O_WRONLY|O_APPEND|O_CREAT|O_NONBLOCK, 0x1a4)) < 0)
                              ArgusLog (LOG_ERR, "ArgusWriteSocket: open(%s, O_WRONLY|O_APPEND|O_CREAT|O_NONBLOCK, 0x1a4) failed %s\n",
                                         asock->filename, strerror(errno));
#ifdef ARGUSDEBUG
                           ArgusDebug (6, "ArgusWriteOutSocket: created outfile %s\n", asock->filename);
#endif
                        }
...
long long RaDiffTime (struct ArgusTime *, struct ArgusTime *, struct timeval *);

long long
RaDiffTime (struct ArgusTime *s1, struct ArgusTime *s2, struct timeval *diff)
{
   long long v1 = 0, v2 = 0;
