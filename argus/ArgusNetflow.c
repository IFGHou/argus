...
struct ArgusRecord *ArgusParseCiscoRecordV9Data (struct ArgusModelerStruct *, void **, u_char *, int);
struct ArgusRecord *ArgusParseCiscoRecordV9Template (struct ArgusModelerStruct *, void **, u_char *, int);
...
struct ArgusRecord *
ArgusParseCiscoRecordV9Data (struct ArgusModelerStruct *model, void **templates, u_char *ptr, int len)
{
   ...
   if (templates != NULL) {
      while (!done) {
         CiscoFlowEntryV9_t *cflow = (CiscoFlowEntryV9_t *) ptr;

         cflow->flowset_id = ntohs(cflow->flowset_id);
         cflow->length = ntohs(cflow->length);

         if (templates[cflow->flowset_id]) {
#ifdef ARGUSDEBUG
            ArgusDebug (5, "ArgusParseCiscoRecordV9Data (%p, %p, %p, %d)\n", model, templates, ptr, len);
#endif
         }

         if ((len - cflow->length) > 0) {
            ptr -= cflow->length;
            len -= cflow->length;
         } else 
            done = 1;
...
#ifdef ARGUSDEBUG
   ArgusDebug (5, "ArgusParseCiscoRecordV9 (%p, %p, %p, %d) returning %p\n", model, templates, ptr, len, retn);
#endif
   return(retn);
}
...
struct ArgusRecord *
ArgusParseCiscoRecordV9Template (struct ArgusModelerStruct *model, void **templates, u_char *ptr, int len)
{
   struct ArgusRecord *retn = NULL;
   int i, done = 0;
   ...
   if (templates != NULL) {
      while (!done) {
         ...
         if (templates[tHdr->template_id] != NULL) {
            ArgusFree(templates[tHdr->template_id]);
            templates[tHdr->template_id] =  NULL;
         }
